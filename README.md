...
本系統採用混合資料庫架構，分別使用 **PostgreSQL** 和 **MongoDB** 來儲存不同類型的資料：

### PostgreSQL (關聯式資料庫)
儲存所有結構化的交易資料和業務邏輯資料：
- **用戶資料**：用戶基本資訊、技能、社交連結
- **K-pop 資料**：歌曲、團體、偶像資訊及其關聯
- **專案資料**：翻跳專案、成員招募、申請記錄、練習排程
- **作品集資料**：用戶上傳的翻跳影片及相關資訊

### MongoDB (NoSQL 資料庫)
儲存非結構化的行為分析資料：
- **行為事件**：用戶點擊、瀏覽、搜尋等行為記錄
- **會話資料**：用戶登入會話的開始/結束時間、裝置資訊
- **行為聚合**：按日期和事件類型聚合的統計資料
