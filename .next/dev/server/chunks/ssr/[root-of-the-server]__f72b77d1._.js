...
function AuthPage() {
    ...
    const handleLogin = async (e)=>{
        ...
        try {
            // 使用用戶名稱和密碼登入
            const { data, error: fetchError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('u_id, status, password, role').eq('name', loginData.name).single();
            if (fetchError || !data) {
                setError('用戶名稱或密碼錯誤');
                setLoading(false);
                return;
    ...
    const handleRegister = async (e)=>{
        ...
        try {
            // 檢查用戶名稱是否已存在
            const { data: existingName } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('name').eq('name', registerData.name).single();
            if (existingName) {
            ...
            const { data: existingUser } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('email, phone').or(`email.eq.${registerData.email},phone.eq.${registerData.phone}`).limit(1);
            ...
            while(attempts < maxAttempts){
                const { data: checkId } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('u_id').eq('u_id', newUserId).single();
                if (!checkId) {
                    break; // ID 不存在，可以使用
                }
            ...
            // 插入新用戶
            const { data: newUser, error: insertError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').insert({
                u_id: newUserId,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f72b77d1._.js.map