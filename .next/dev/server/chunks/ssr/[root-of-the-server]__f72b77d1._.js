...
function AuthPage() {
    ...
    const handleLogin = async (e)=>{
        ...
        try {
            // 使用用戶名稱和密碼登入
            const { data, error: fetchError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('u_id, status, password').eq('name', loginData.name).single();
            if (fetchError || !data) {
                setError('用戶名稱或密碼錯誤');
                setLoading(false);
                return;
